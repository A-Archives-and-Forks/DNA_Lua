...
function SetVarNode:Execute()
  if not self.VarName or self.VarName == "" then
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "SetVarNode节点出错", "没有填写VarName, FileName:" .. tostring(self.Context.FileName) .. ",请策划排查.")
    return
  end
  ...
  if not VarInfo then
    ...
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "SetVarNode节点出错", _Str)
    return
  end
