...
function PrintVarNode:Execute()
  if not self.VarName or self.VarName == "" then
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "PrintVarNode节点出错", "没有填写VarName, FileName:" .. tostring(self.Context.FileName) .. ",请策划排查.")
    return
  end
  ...
  if not VarInfo then
    ...
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "PrintVarNode节点出错", _Str)
    return
  end
  if VarInfo.QuestChainId and VarInfo.QuestChainId ~= self.QuestChainId then
    ...
    _Str = _Str .. ",请策划排查."
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "StorySubsystem错误", _Str)
    return
  end
