...
function ExecuteBlueprintFunctionSetVarNode:Execute()
  if not self.VarName or self.VarName == "" then
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "ExecuteBlueprintFunctionSetVarNode节点出错", "没有填写VarName, FileName:" .. tostring(self.Context.FileName) .. ",请策划排查.")
    return
  end
  ...
  if not VarInfo then
    ...
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "ExecuteBlueprintFunctionSetVarNode节点出错", _Str)
    return
  end
  local NewVarInfos = {}
  ...
  if type(Ret) ~= "number" or 0 ~= Ret % 1 then
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "ExecuteBlueprintFunctionSetVarNode节点出错", "函数[" .. tostring(self.FunctionName) .. "]的返回值不是int类型")
    return nil
  end
