...
function ExecuteBlueprintFunctionCheckVarNode:Execute(Callback)
  ...
  if not self.VarName or self.VarName == "" then
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "ExecuteBlueprintFunctionCheckVarNode节点出错", "没有填写VarName, FileName:" .. tostring(self.Context.FileName) .. ",请策划排查.")
    return
  end
  if self.Duration < 0 then
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "ExecuteBlueprintFunctionCheckVarNode节点出错", "持续时间只能填0和正整数, 持续时间: " .. self.Duration .. ", FileName:" .. tostring(self.Context.FileName) .. ",请策划排查.")
    return
  end
  ...
  if not VarInfo then
    ...
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "ExecuteBlueprintFunctionCheckVarNode节点出错", _Str)
    return
  end
  self.NewVarInfos = {}
  ...
  if type(Ret) ~= "number" or 0 ~= Ret % 1 then
    UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, "ExecuteBlueprintFunctionCheckVarNode节点出错", "函数[" .. tostring(self.FunctionName) .. "]的返回值不是bool类型")
    return nil
  end
  if 0 == self.Duration then
    return 1 == Ret
